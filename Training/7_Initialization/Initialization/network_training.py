...
def train_network(parameters, data, epochs=1000, batch_size=32, loss='mse', verbose=False, seed=None, use_min_perf=False, callbacks=[], model_path='', model_storage='', save_threshold=None, current_perf=0):
    ...
    if save_threshold != None:
        p = current_perf + last_perf
        p /= 5

        if p < save_threshold:
            print('model saved.', p,save_threshold)
            save_model(model, 'best_model.mh5')
        else:
            print('model not good enough.',p,save_threshold)
    else:
        print('model not last cv.','n/a',save_threshold)
